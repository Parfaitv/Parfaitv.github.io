...
export default function TaskList({ tasks, onTaskToggle, onTaskDelete }: TaskListProps) {
  return (
    <div className="bg-white rounded-lg shadow-md p-4">
      ...
      {tasks.length === 0 ? (
        <div className="text-center py-8 bg-gray-50 rounded-lg">
          ...
          <p className="text-light-text">Задачи не найдены. Добавьте новую задачу, чтобы начать!</p>
        </div>
      ) : (
              ...
              >
                <Checkbox
                  ...
                  className={`w-5 h-5 mr-3 rounded-full border-2 ${
                    task.completed ? 'border-completed text-completed' : 'border-primary text-primary'
                  }`}
                />
                <span 
                  className={`flex-grow text-dark-text ${
                    task.completed ? 'line-through text-light-text' : 'text-dark-text'
                  }`}
                >
                  {task.text}
                </span>
